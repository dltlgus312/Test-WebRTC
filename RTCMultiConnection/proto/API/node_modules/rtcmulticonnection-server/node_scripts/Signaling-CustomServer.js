...
module.exports = exports = function CustomServer(io) {
	...
	this.addSocket = function(socket){
		...
		console.log( 'Rtc CustomServer ::: Signaling-CustomServer.addSocket' );
		console.log( 'Rtc CustomServer ::: USER Connection ID = ' + params.userid );
		console.log( 'Rtc CustomServer ::: ROOM Connection ID = ' + params.sessionid );
		...
		socket.on('fileUpload', function(data){
			data.forEach(function(d){
				try{
					var exe = d.name.split('.');
					
					var wstream = fs.createWriteStream( path + d.uuid + '.' + exe[exe.length-1] );
					
					wstream.write(d.data);
					...
					socket.emit('fileUploadResult', d);
					
					socket.in(sessionid).emit('fileUploadResult', d);
