...
(function() {
    ...
    // ## Shape List For Drag ##
    function addShapeOptions(index, shape){
		
		shapeSelectList[index] = shape;
		
    	var shapeContainer = find('shape-select-container');
    	
    	var div = document.createElement('div');
    	
    	div.setAttribute('class', 'shape-item');
    	...
    	div.onmouseover = function(){
    		...
    		dragHelper.init(div.firstChild.value);
    	}
    	...
    	div.onmouseout = function(){
    		...
    		tempContext.clearRect(0, 0, innerWidth, innerHeight);
    		dragHelper.init(selectedIndex);
    	}
    	
    	div.onclick = function(){
    		selectedIndex = div.firstChild.value;
    		
    		var nodes = shapeContainer.childNodes;
    		for(var i=0;i<nodes.length;i++){
    			if (nodes[i].nodeName.toLowerCase() == 'div') {
    		         nodes[i].style.background = '#00000000';
    		         nodes[i].firstChild.selected = false;
    		     }
    		}
    	...
    	shapeContainer.insertBefore(div, shapeContainer.firstElementChild);
    	
    }
    
    function removeShapeOptions(type){
    	var shapeContainer = find('shape-select-container');
    	if(type == 'all'){
    		shapeContainer.innerHTML = '';
    	}else {
    		shapeContainer.removeChild(shapeContainer.firstElementChild);
    		shapeContainer.firstElementChild ? shapeContainer.firstElementChild.onclick() : null;
    	}
    }
    
    // ## Saved Points get
    function loadShapeSelect(selectList){
    	var shapeContainer = find('shape-select-container');
    	shapeContainer.innerHTML = '';
    	
    	if(selectList){
    		for(var k in shapeSelectList){
    			if(shapeSelectList[k] == null) continue;
    			addShapeOptions(~~k, shapeSelectList[~~k]);
    		}
    	}else {
    		var key = openPdf ? pdfHandler.openedKey : 'p';
    		var page = openPdf ? pdfHandler.pdfPagesList.selectedIndex : nowPage;
    		
    		shapeSelectList = savePoints[key][page][2];
    ...
    // ## Save Points ##
    function pointsSave(type, isClaer){
    	
    	var pdf;
    	
    	if(type=='p'){
    		// pdf 를 불러오기 전에 그린 그림 저장
    		!savePoints['p'] ? savePoints['p'] = [] : '';
    		!savePoints['p'][nowPage] ? savePoints['p'][nowPage] = [] : '';
    		
        	savePoints['p'][nowPage][0] = points;
        	savePoints['p'][nowPage][1] = pencilUndo;
        	savePoints['p'][nowPage][2] = shapeSelectList;
    	}else {
			savePoints[pdfHandler.openedKey][pdfHandler.prePageNumber] = [];
			
			savePoints[pdfHandler.openedKey][pdfHandler.prePageNumber][0] = points;
			savePoints[pdfHandler.openedKey][pdfHandler.prePageNumber][1] = pencilUndo;
			savePoints[pdfHandler.openedKey][pdfHandler.prePageNumber][2] = shapeSelectList;
			
			// ## pdf 뺴고 저장
			pdf = savePoints[pdfHandler.openedKey][pdfHandler.prePageNumber][0].splice(0,1);
			
			// ## 빈값 없애기 (그렸다 지울경우)
			if(savePoints[pdfHandler.openedKey][pdfHandler.prePageNumber][0].length == 0){
				savePoints[pdfHandler.openedKey][pdfHandler.prePageNumber] = null;
			}
    	}
    	...
    	if(isClaer){
    		...
    		pencilUndo.length = 0;
    		shapeSelectList = [];
    		shapeSelectList.length = 0;
    		removeShapeOptions('all');
    	}else {
    		if(pdf){
    			points = savePoints[pdfHandler.openedKey][pdfHandler.prePageNumber] ? pdf.concat(savePoints[pdfHandler.openedKey][pdfHandler.prePageNumber][0]) : pdf;
    		}
    	}
    ...
    function loadPaging(){
    	var pageContainer = find('page-container');
    	var pagePrev = find('page-prev');
    	var pageList = find('page-list');
    	var pageNext = find('page-next');
    	var pageClose = find('page-close');
    	...
    	for(var i=0;i<=totalPage;i++){
    		...
    		option.value = i+1;
    		option.innerHTML = 'Page #' + (i+1) + "/" + (totalPage+1);
    	...
    	pagePrev.src = data_uris.pdf_next;
    	pageNext.src = data_uris.pdf_prev;
    	pageClose.src = data_uris.pdf_close;
    	
    	pageContainer.style.position = 'fixed';
    	pageContainer.style.bottom = '30px';
    	pageContainer.style.left = '100px';
    	pageContainer.style.display = 'block';
    	
    	pagePrev.onclick = function(){
    		if(pageList.selectedIndex-1 == -1){
    	...
    	pageNext.onclick = function(){
    		if(pageList.selectedIndex+1 == pageList.length){
    	...
    	pageList.onchange = function(){
    		pointsSave('p', true);
    		nowPage = pageList.selectedIndex;
			points = savePoints['p'][nowPage][0];
			pencilUndo = savePoints['p'][nowPage][1];
			shapeSelectList = savePoints['p'][nowPage][2];
			saveDo = [];
    	...
    	pageClose.onclick = function(){
    		savePoints['p'].splice(nowPage, 1);
    		
    		if(!savePoints['p'][nowPage]) nowPage--;
    ...
    var dragHelper = {
        ...
        mousedown: function(e) {
            ...
            if (points.length) {
            	// ## last path
                var p = points[selectedIndex == '' ? points.length - 1 : selectedIndex - 1];
...
//                    }
        ...
        mousemove: function(e) {
            ...
            // ## last path
            if (is.isDragLastPath) this.init(selectedIndex == '' ? points.length - 1 : selectedIndex);
        },
        init: function(index) {
			if (!points.length || !shapeSelectList.length || !points[index - 1] || !points[index - 1][1]) return;

            // ## last path
            var p = points[index - 1],
                point = p[1],
                g = this.global;
...
//                tempContext.fill();
        ...
        dragShape: function(x, y) {
           ...
           if (!points.length) return;
            ...
            // ## last path
            var point = points[tools.dragSingle == true && selectedIndex != '' ? selectedIndex - 1 : points.length - 1 ];
...
//                    ];
			...
			// ## Save Do Drag All Path ##
            for (var j = 0; j < saveDo.length; j++) {
            	p = saveDo[j];
...
//                        ];
        ...
        dragLastPath: function(x, y) {
        	...
        	// ## last path
            if (selectedIndex == 0 || !points[selectedIndex - 1]) return;

            var g = this.global,
                prevX = g.prevX,
                prevY = g.prevY,
                // ## last path
                p = points[selectedIndex == '' ? points.length - 1 : selectedIndex - 1],
                point = p[1],
                getPoint = this.getPoint,
            ...
            if (p[0] === 'line') {
                ...
                // ## last path
                points[selectedIndex == '' ? points.length - 1 : selectedIndex - 1] = [p[0], point, p[2]];
            }
...
//                points[points.length - 1] = [p[0], point, p[2]];
    ...
    var pencilHandler = {
        ...
        mousedown: function(e) {
            ...
            pencilHandler.startIndex = points.length; 
        },
        mouseup: function(e) {
            ...
            // ## Pencil Undo 
            pencilHandler.endIndex = points.length;
            
            pencilUndo[pencilHandler.endIndex] = {
            		startIndex : pencilHandler.startIndex - 1
            }
        },
        mousemove: function(e) {
            var x = e.pageX - canvas.offsetLeft,
    ...
    var eraserHandler = {
        ...
        mousedown: function(e) {
            ...
            eraserHandler.startIndex = points.length; 
        },
        mouseup: function(e) {
            this.ismousedown = false;
            
            // ## Pencil Undo 
            eraserHandler.endIndex = points.length;
            
            pencilUndo[eraserHandler.endIndex] = {
            		startIndex : eraserHandler.startIndex - 1
            }
        },
        mousemove: function(e) {
            var x = e.pageX - canvas.offsetLeft,
    ...
    // ########## 지우개 색상 고정 ##########
    var eraserLineWidth = 10,
    eraserStrokeStyle = '#ffffff',
    eraserGlobalAlpha = 1.0;

	var eraserDrawHelper = clone(drawHelper);
	
	eraserDrawHelper.getOptions = function() {
	    return [eraserLineWidth, eraserStrokeStyle, fillStyle, eraserGlobalAlpha, globalCompositeOperation, lineCap, lineJoin, font];
	}
    ...
    var arcHandler = {
        ...
        mouseup: function(e) {
            ...
            if (g.ismousedown) {
                ...
                // ## addShapeOptions
                addShapeOptions(points.length, 'arc');
            }
    ...
    var lineHandler = {
        ...
        mouseup: function(e) {
            ...
            if (t.ismousedown) {
                ...
                // ## addShapeOptions
                addShapeOptions(points.length, 'line');
            }
        },
        mousemove: function(e) {
            var x = e.pageX - canvas.offsetLeft,
    ...
    var arrowHandler = {
        ...
        mouseup: function(e) {
            ...
            if (t.ismousedown) {
                ...
                // ## addShapeOptions
                addShapeOptions(points.length, 'arrow');
            }
        },
        mousemove: function(e) {
            var x = e.pageX - canvas.offsetLeft,
    ...
    var rectHandler = {
        ...
        mouseup: function(e) {
            ...
            if (t.ismousedown) {
                ...
                // ## addShapeOptions
                addShapeOptions(points.length, 'rect');
            }
    ...
    var imageHandler = {
        ...
        load: function(width, height) {
            ...
            // ## click();
            document.getElementById('pencil-icon').click();
        ...
        mouseup: function(e) {
            ...
            if (t.ismousedown) {
                ...
                t.ismousedown = false;
                // ## addShapeOptions
                addShapeOptions(points.length, 'image');
            }
    ...
    var pdfHandler = {
        ...
        // ## Saved Points Draw
        afterLoadImages : function(){
        	try{
        		if(!!savePoints[pdfHandler.openedKey][pdfHandler.pdfPagesList.selectedIndex]){
        			// ## Twise Call Error : Only One Concat  
        			points = points.length == 1 ? points.concat(savePoints[pdfHandler.openedKey][pdfHandler.pdfPagesList.selectedIndex][0]) : points;
        			pencilUndo = savePoints[pdfHandler.openedKey][pdfHandler.pdfPagesList.selectedIndex][1];
        			shapeSelectList = savePoints[pdfHandler.openedKey][pdfHandler.pdfPagesList.selectedIndex][2];
        			saveDo = [];
        ...
        load: function(lastPdfURL) {
        	...
        	// ## Saved Points
        	if((pdfHandler.pdfPagesList.value == '' || pdfHandler.pdfPagesList.value == null) && !openPdf){
        		pointsSave('p', true);
        	}
            ...
            pdfHandler.getPage(parseInt(pdfHandler.pdfPagesList.value || 1), function(lastPage, width, height, numPages) {
                ...
                var realWidth = find('temp-canvas').offsetWidth - find('tool-box').offsetWidth;
                var realHeight = find('temp-canvas').offsetHeight;

                var t = pdfHandler;
                pdfHandler.lastIndex = pdfHandler.images.length;
                var point = [lastPage, 40, find('pencil-container').offsetHeight, realWidth, realHeight, pdfHandler.lastIndex];
                ...
                // ## Saved Points 이전 페이지 기억
                pdfHandler.prePageNumber = pdfHandler.pdfPagesList.selectedIndex;
                
                enableTouch = true;
                
                pdfHandler.pdfPagesList.onchange = function() {
                	pointsSave(null, true);
                ...
                pdfHandler.pdfNext.onmouseover = function(){
                	pdfHandler.pdfNext.style.cursor = "pointer";
                }
                ...
                pdfHandler.pdfPrev.onmouseover = function(){
                	pdfHandler.pdfPrev.style.cursor = "pointer";
                }
                ...
                pdfHandler.pdfClose.onclick = function() {
                	...
                	var selectPdfList = find('pdf-select-list');
	                selectPdfList.selectedIndex = '0';
                    
                    // ## Save Points ##
	                pointsSave(null, true);
                    pdfHandler.prePageNumber = 1;
                    pdfHandler.openedKey = null;
                ...
                pdfHandler.pdfClose.onmouseover = function(){
                	pdfHandler.pdfClose.style.cursor = "pointer";
                }
                ...
                pdfHandler.pdfPageContainer.style.bottom = '30px';
                pdfHandler.pdfPageContainer.style.left = (point[3] - parseInt(point[3] / 1.85)) + 'px';
...
//            pdfHandler.pdfPageContainer.style.left = (point[1] + point[3] - parseInt(point[3] / 2) - parseInt(pdfHandler.pdfPageContainer.clientWidth / 2)) + 'px';
    ...
    var data_uris = {
        ...
        line: icons.line || 'data:image/png;base64,',
        ...
        arrow: icons.arrow || 'data:image/png;base64,',
        ...
        pencil: icons.pencil || 'data:image/png;base64,',
        ...
        dragMultiple: icons.dragMultiple || 'data:image/png;base64,',
        ...
        eraser: icons.eraser || 'data:image/png;base64,',
        ...
        rectangle: icons.rectangle || 'data:image/png;base64,',
        ...
        arc: icons.arc || 'data:image/png;base64,',
        ...
        pdf: icons.pdf || 'data:image/png;base64,',
        ...
        undo: icons.undo || 'data:image/svg+xml;base64,',
        do: icons.do || 'data:image/png;base64,',
		// ## clear: icons.clear || 'data:image/png;base64,',
        clear: icons.clear || 'data:image/png;base64,',
        ...
        pdf_close: icons.pdf_close || 'data:image/png;base64,',
        fullScreen: icons.fullScreen || 'data:image/png;base64,',
        paging: icons.paging || 'data:image/png;base64,'
    };
    ...
    function getColorStyle(){
    	var colorPickerContainer = find('pencil-fill-colors'),
        ...
        lineWidth = strokeStyleText.value;
        strokeStyle = hexToRGBA(fillStyleText.value, alpha);
    }
    
    function getEraserStyle(){
		var strokeStyleText = find('eraser-stroke-style');
    	eraserLineWidth = strokeStyleText.value;
    }
    
    function showColorStyle(){
    	var colorContainer = find('pencil-container');
    ...
    (function() {
        ...
        function decorateDragLastPath() {
            ...
            image.onload = function() {
                ...
                addEvent(context.canvas, 'click', function(){
                	...
                	shapeContainer.style.left = (context.canvas.offsetLeft + context.canvas.clientWidth) + 'px';
                	dragHelper.init(selectedIndex);
                });
            ...
            image.onload = function() {
                ...
                document.querySelector('#undo').onclick = function() {
                    ...
                    if (points.length) {
                    	...
                    	// ## Pencil Undo
                    	if(pencilUndo[points.length]){
                    		
                    		var index = points.length;
                    		
                    		var cpLength = index - pencilUndo[index].startIndex;
                    		
                    		var newArray = points.splice(pencilUndo[index].startIndex, cpLength);
                    		
                    		saveDo[saveDo.length] = newArray;
                    		
                    		points.length = pencilUndo[index].startIndex;
                    		
                    		pencilUndo.splice(index, 1);
                    	}else {
                    		if( (openPdf && points.length == 1) || (points[0][0] == 'pdf' && points.length == 1) ){
                    			pencilUndo = [];
                    			pencilUndo.length = 0;
                    			shapeSelectList = [];
                    			shapeSelectList.length = 0;
							...
							saveDo[saveDo.length] = [points[index - 1]];

                    		points.length = index - 1;
                    		
                    		if(shapeSelectList[index]){
                    			shapeSelectList.splice(index, 1);
                    			removeShapeOptions();
                    		}
        	...
        	image.onload = function() {
        		...
        		addEvent(context.canvas, 'click', function(){
        			...
        			if(saveDo.length != 0){
        				...
        				points = points.concat(saveDo[saveDo.length-1]);

        				if(saveDo[saveDo.length-1].length > 1){
        					pencilUndo[points.length] = { startIndex : startIndex }
        				}else if(tools.dragSingle === true){
							addShapeOptions(points.length, points[points.length - 1][0]);
						}
        				
        				saveDo.splice(saveDo.length-1, 1);
        ...
        function decorateFullScreen() {
        	var context = getContext('fullScreen');
        	...
        	image.src = data_uris.fullScreen;
        }
        
        if (tools.fullScreen === true) {
        	decorateFullScreen();
        	document.getElementById('fullScreen').style.display = 'block';
        }

        function decoratePaging() {
        	var context = getContext('paging');
        	
        	var image = new Image();
        	image.onload = function() {
        		context.drawImage(image, 4, 4, 32, 32);
        		
        		document.querySelector('#paging').onclick = function() {
        			if(openPdf){
        				alert('PDF 파일에선 불가능합니다.');
        			}else {
        				
        				pointsSave('p', true);

        				nowPage = totalPage = totalPage+1;
        	...
        	image.src = data_uris.paging;
        }
        
        if (tools.paging === true) {
        	decoratePaging();
        ...
        function decoratePencil() {
            ...
            image.onload = function() {
             ...
             // ########## 펜 색상 통일 ##########
                ...
                var colorContainer = find('pencil-container'),
                	colorPickerContainer = find('pencil-fill-colors'),
                    strokeStyleText = find('pencil-stroke-style'),
                    colorColorsList = find("pencil-colors-list"),
                    fillStyleText = find('pencil-fill-style'),
                    colorSelectedColor = find('pencil-selected-color'),
                    // colorSelectedColor2 = find('pencil-selected-color-2'),
                    // btnColorDone = find('pencil-done'),
                    canvas = context.canvas,
                    alpha = 0.2;
                
                strokeStyle = hexToRGBA(fillStyleText.value, alpha);
                
                colorContainer.style.left = (canvas.offsetLeft + canvas.clientWidth) + 'px';
            	
                colorSelectedColor.style.backgroundColor = '#' + fillStyleText.value;
                // colorSelectedColor2.style.backgroundColor = '#' + fillStyleText.value;
                ...
                colors.forEach(function(colorRow) {
                    ...
                    colorColorsList.innerHTML += row;
                })
                
                Array.prototype.slice.call(colorColorsList.getElementsByTagName('td')).forEach(function(td) {
                    addEvent(td, 'mouseover', function() {
                        var elColor = td.getAttribute('data-color');
                        // colorSelectedColor2.style.backgroundColor = '#' + elColor;
                        colorSelectedColor.style.backgroundColor = '#' + elColor;
                ...
                // ### Custom UI ###
                // addEvent(btnColorDone, 'click', function() {
                // 	getColorStyle();
                // });
                
                addEvent(colorSelectedColor, 'click', function() {
                	colorPickerContainer.style.display = 'block';
                	colorPickerContainer.style.position = 'fixed';
                	// colorPickerContainer.style.top = (canvas.offsetTop + 1) + 'px';
                	colorPickerContainer.style.top = colorContainer.offsetHeight + 'px';
                	colorPickerContainer.style.left = (canvas.offsetLeft + canvas.clientWidth) + 'px';
                });
                
                colorSelectedColor.onmouseover = function(){
                	colorSelectedColor.style.cursor = "pointer";
                }
                
                fillStyleText.onchange = function(){
                	colorSelectedColor.style.backgroundColor = '#' + fillStyleText.value;
        ...
        function decorateEraser() {
            ...
            image.onload = function() {
                ...
                // ############# 지우개 크기 설정 #############.
                var eraserContainer = find('eraser-container'),
                strokeStyleText = find('eraser-stroke-style'),
                // btnColorsDone = find('eraser-done'),
                canvas = context.canvas;
                
                addEvent(canvas, 'click', function() {
                	eraserContainer.style.display = 'block';
                	eraserContainer.style.left = (canvas.offsetLeft + canvas.clientWidth) + 'px';
        ...
        function decoratePDF() {
            ...
            var selectContext = getContext('pdf-icon');

            var image = new Image();
            image.onload = function() {
                
            	var selectPdfList = find('pdf-select-list');
            	var selectPdf = find('pdf-select');
            	var selectPdfSave = find('pdf-save');
                ...
                // #################################### Custom PDF UI ############################################
                addEvent(context.canvas, 'click', function() {
	        	    var selectContainer = find('pdf-select-container');
	                selectContainer.style.display = 'block';
	                selectContainer.style.left = (context.canvas.offsetLeft + context.canvas.clientWidth) + 'px';
                });
                

                function onGettingPdf(file) {
                    ...
                    reader.onload = function(event) {
                        ...
                        pdfHandler.pdfPagesList.value = '';
                        
	                    // ######### 파일 기억 ##########
                        pdfHandler.openedKey = file.name;

                        if(!fileList[file.name]){
		                   	// ## Init ##
                        	fileList[file.name] = file;
		                   	savePoints[file.name] = [];
		                   	// ##########
	                   	}
                        // ###########################
                ...
                function doSavePdf(key){
                	var file = fileList[key];
                	var p = savePoints[key];
                ...
                selectPdf.onclick = function(){
                     ...
                     selector.selectSingleFile(function(file) {
	                    ...
	                    selectPdfList.appendChild(option);
	                    option.selected = true;
                ...
                selectPdfList.onchange = function(){
                	if(this.value !== 'empty'){
                ...
                selectPdfSave.onclick = function(){
            	
					if(openPdf){
						pointsSave(null, false);
					}
                	  
                	doSavePdf(pdfHandler.openedKey);
                };
        ...
        function decorateColors() {
            ...
            colorSelectedColor.style.backgroundColor =
            	colorSelectedColor2.style.backgroundColor = '#' + fillStyleText.value;
    ...
    function hideContainers() {
        ...
        var additionalContainer = find('additional-container'),
            ...
            pdfSelectContainer = find('pdf-select-container'),
            shapeSelectContainer = find('shape-select-container'),
            markerContainer = find('marker-container'),
            markerColorContainer = find('marker-fill-colors'),
            pencilContainer = find('pencil-container'),
            eraserContainer = find('eraser-container'),
    ...
    function onkeyup(e) {
        ...
        if (isControlKeyPressed && keyCode === 90) {
        	...
        	if(pencilUndo[points.length]){
        		...
        		if( (openPdf && points.length == 1) || (points[0] && points[0][0] == 'pdf' && points.length == 1) ){
        			pencilUndo = [];
        			pencilUndo.length = 0;
        			shapeSelectList = [];
        			shapeSelectList.length = 0;
        		...
        		if(shapeSelectList[points.length]){
        			shapeSelectList.splice(points.length, 1);
        			removeShapeOptions();
        		}
    ...
    window.addEventListener('message', function(event) {
        ...
        if (event.data.enableTouch) {
        	enableTouch = event.data.enable;
        	return;
        }
        
        if (event.data.doSave) {
        	var data;
        	...
        	if(event.data.doSave == 'all'){
        		// ## 모든 데이터
        		data = savePoints;
        	}else if(event.data.doSave.key){
        		// ## 원하는 곳의 데이터
        		var key = event.data.doSave.key;
        		var page = event.data.doSave.page;
        		
        		data = savePoints[key][page ? page : 0];
        	}else if(event.data.doSave == 'now'){
        		// ## PDF 뺸 현재 페이지
        		var key = openPdf ? pdfHandler.openedKey : 'p';
        		var page = openPdf ? pdfHandler.pdfPagesList.selectedIndex : nowPage;
    			data = savePoints[key][page];
        	}else {
        		// ## PDF 포함한 현재 페이지
				data = {
					points : points,
					pencilUndo : pencilUndo,
					shapeSelectList : shapeSelectList
    			}
        	}
        ...
        if (d.startIndex && d.startIndex !== 0) {
        ...
        pencilUndo = d.pencilUndo;
        saveDo = d.saveDo;
		
        // ## ui setting ##
        if(tools.dragSingle === true){
        	shapeSelectList = d.shapeSelectList;
        	loadShapeSelect(shapeSelectList);
        }
    ...
    function syncPoints(isSyncAll, isPlzSync) {
        if (isSyncAll) {
            lastPointIndex = 0;
        }

        // @@ 방장만 서버로부터 데이터를 받아 셋팅 유저는(빈데이터로 시작해서) 방장으로부터 sync 맞춤
        // ############### Last Path Remove Error ###############
        if(isPlzSync){
