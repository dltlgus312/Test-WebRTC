...
//                points[points.length - 1] = [p[0], point, p[2]];
    ...
    var pdfHandler = {
        ...
        // ## Save Points ##
        pdfSavePoints: function (type){
        	if(type=='p'){
        		// pdf 를 불러오기 전에 그린 그림 저장
        		savePoints['p'] = [];
            	
            	savePoints['p'][0] = points;
            	savePoints['p'][1] = pencilUndo;
        	}else {
        		if(pencilUndo.length != 0){
        			savePoints[pdfHandler.openedKey][pdfHandler.prePageNumber] = [];
        			
        			savePoints[pdfHandler.openedKey][pdfHandler.prePageNumber][0] = JSON.parse(JSON.stringify(points));
        			savePoints[pdfHandler.openedKey][pdfHandler.prePageNumber][1] = JSON.parse(JSON.stringify(pencilUndo));
        			
        			// ## pdf는 뺴고 저장
        			savePoints[pdfHandler.openedKey][pdfHandler.prePageNumber][0].splice(0,1);
        			
        			// ## 빈값 없애기 (그렸다 지울경우)
        			if(savePoints[pdfHandler.openedKey][pdfHandler.prePageNumber][0].length == 0){
        				// savePoints[pdfHandler.openedKey].splice(pdfHandler.prePageNumber, 1, []);
        ...
        load: function(lastPdfURL) {
            ...
            pdfHandler.getPage(parseInt(pdfHandler.pdfPagesList.value || 1), function(lastPage, width, height, numPages) {
                ...
                pdfHandler.pdfClose.onclick = function() {
                    ...
                    points = savePoints['p'][0];
                	pencilUndo = savePoints['p'][1];
