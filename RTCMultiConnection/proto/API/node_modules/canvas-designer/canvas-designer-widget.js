...
function CanvasDesigner() {
    ...
    function onMessage(event) {
    	...
    	// ## Save Data {file, canvas}
    	if(!!event.data.doSave){
    		designer.ondosave(event.data.doSave);
    		return;
    	}

    	// ## File Share
    	if(!!event.data.fileShare){
        	designer.onaddfileshare(event.data.fileShare);
        	return;
        }
        ...
        if (!!event.data.dataURL) {
            ...
            designer.ondataurl(event.data.dataURL);
            return;
        }
    ...
    designer.toDataURL = function(format) {
        if (!designer.iframe) return;
        designer.postMessage({
            genDataURL: true,
            format: format
    ...
    designer.enableTouch = function (enable) {
    	if (!designer.iframe) return;
