...
var RTCMultiConnection = function(roomid, forceOptions) {
    ...
    function SocketConnection(connection, connectCallback) {
        ...
        // ## because share coding socket is dynamic url
//        var parameters = '';
//
...
//        }
        ...
        try {
        	...
        	connection.socketOptions.query = parameters;
            connection.socket = io.connect(connection.socketURL, connection.socketOptions);
        } catch (e) {
			console.error(e);
        }
        ...
        // ## fileUpload in server ##
        connection.socket.on('fileUploadResult', function(data){
        	if(data.result === 'SUCCESS'){
        		rtc.conn.addFile(data);
        	}else {
        		connection.onnoticemessage('', 'FileUploadFail', data.error);
        	}
    	});
        // ##########################
        ...
        function onMessageEvent(message) {
			...
			// ## Cluster (Redis) Custom Server Interop
        ...
        connection.socket.on('userid-already-taken', function(useridAlreadyTaken, yourNewUserId) {
            // ## onUserIdAlreadyTaken && Id Change
    ...
    function MultiPeers(connection) {
            ...
            return {
                ...
                onDataChannelMessage: function(message) {
                    ...
                    if (typeof message == 'string' || !connection.enableFileSharing) {
                    	// ## 참고 : Primary Message & Canvas Image, pdf......
			...
			if (!!peer.getSenders && typeof peer.getSenders === 'function' && peer.getSenders().length) {
				peer.getSenders().forEach(function(rtpSender) {
					if(targetTrack.id === rtpSender.track.id){
						...
						} else if (!isVideoTrack && rtpSender.track.kind === 'audio') {
        ...
        function initFileBufferReader() {
            ...
            connection.fbr.addFile = function(file) {
            	connection.addFile(file);
            };
        }

        this.shareFile = function(file, remoteUserId) {
        	// ## Multi File Share Error
	...
	// ## Find Stream id in Remote SDP ( FireFox : Stream id Changed )
	function getMsid(mid){
		var sdps = this.sdp.split('\n');
		var mids = [];
		sdps.forEach(function(sdp, index){
			if(sdp.indexOf('mid:')>0){
				mids[sdp.substring(sdp.indexOf('mid:')+4, sdp.length-1)] = index;
			}
		});
		
		// console.log(mids, "mid:"+mid, "value:"+mids[mid]);
		
		for(var i = mids[mid]; i < sdps.length; i++){
			if( sdps[i].indexOf('msid:') > 0 ){
				var value = sdps[i].substr(sdps[i].indexOf('msid:'));
    ...
    function PeerInitiator(config) {
		...
		// ## Find Stream id in Remote SDP ( FireFox : Stream id Changed )
		RTCSessionDescription.prototype.getMsid = getMsid;
        ...
        peer.ontrack = function(event) {
			...
			// ## Find Stream id in Remote SDP ( FireFox : Stream id Changed )
			// ## Edge Not Found Object Error ( currentTarget )
			var remoSdp, streamId;
			if(event.currentTarget && event.currentTarget.remoteDescription){
				remoSdp = event.currentTarget.remoteDescription;
				streamId = remoSdp.getMsid(event.transceiver.mid);				
			}
			
			// var streamToShare = streamsToShare[event.stream.id];
            var streamToShare = streamsToShare[event.stream.id] ? streamsToShare[event.stream.id] : remoSdp ? streamsToShare[streamId] : null;
            ...
            event.stream.streamid = streamId;
        ...
        function createOfferOrAnswer(_method) {
            peer[_method](defaults.sdpConstraints).then(function(localSdp) {
                ...
                if (!!connection.enableLogs) {
                	// @@ log ( onconnectionstatechange 추가로 인한 에러 발생 : 무시 )
    ...
    function TextReceiver(connection) {
        ...
        function receive(data, userid, extra) {
            ...
            if (data.last) {
                ...
                // ## Notice Event ( RTC.js 에서 처리 )
            	// connection.onnoticemessage(userid, null, message);
    ...
    var FileProgressBarHandler = (function() {
        function handle(connection) {
            ...
            // ## Custom UI ##
            connection.onFileStart = function(file, isProvider) {
                ...
                div.id = 'sharing_' + file.uuid;

                div.setAttribute('class', 'fileSharing sharing_' + file.userid + ' sharing_' + file.remoteUserId + (file.provideId ? ' sharing_' + file.provideId : '') );
                
                div.innerHTML = '<div style="width:100%"><progress></progress><label style="float:right">0%</label><input type="hidden" value=' + file.uuid + '></div>';

                if (file.remoteUserId) {
                    div.innerHTML += '<div style="width:100%">';
                    	div.innerHTML += '베포자 : ' + file.userid + '</br>';
	                    div.innerHTML += '파일명 : ' + file.name + '</br>';
	                    div.innerHTML += '파일크기 : ' + formatSizeUnits(~~file.size) + '</br>';
	                    div.innerHTML += '공유대상 : ' + (file.remoteUserId === connection.userid ? ( file.provideId ? file.provideId : file.userid ) : file.remoteUserId) + '</br>';
                    div.innerHTML += '</div>';
                }
                ...
                if(!progressContainer){
                	...
                	progressContainer.style.right = '0px';

                	progressContainer.style.width = '210px';

                	progressContainer.style.maxHeight = '100%';

                	progressContainer.style.overflow = 'auto';
                	
                	progressContainer.style.overflowX = 'hidden';
                	
                	progressContainer.style.boxShadow = 'inset 0 0 14px rgb(229, 229, 229)';
                	
                	progressContainer.style.background = '#00000088';
                	
                	if (!connection.filesContainer) {
                		connection.filesContainer = document.body || document.documentElement;
                	}
                	connection.filesContainer.insertBefore(progressContainer, connection.filesContainer.firstChild);
                }
            ...
            {
                if ( ( bytes >> 30 ) & 0x3FF )
                    bytes = ( bytes >>> 30 ) + '.' + ((bytes & (3*0x3FF)) + '' ).substr(0, 1) + 'GB' ;
                else if ( ( bytes >> 20 ) & 0x3FF )
                    bytes = ( bytes >>> 20 ) + '.' + ((bytes & (2*0x3FF)) + '' ).substr(0, 1) + 'MB' ;
                else if ( ( bytes >> 10 ) & 0x3FF )
                    bytes = ( bytes >>> 10 ) + '.' + ((bytes & (0x3FF)) + '' ).substr(0, 1) + 'KB' ;
            ...
            // ############ file add ###############
            connection.addFile = function(file, container) {
            	...
            	if(!!document.getElementById('shared_' + file.uuid + file.userid)) return;
            	...
            	if(!table){
            		...
            		var fileDate = document.createElement('th');
            		var userName = document.createElement('th');
            		var fileView = document.createElement('th');
            		var fileLink = document.createElement('th');
            		
            		fileName.innerHTML = '파일명';
            		fileSize.innerHTML = '크기';
            		fileDate.innerHTML = '날짜';
            		userName.innerHTML = '베포자';
            		fileView.innerHTML = '미리보기';
            		fileLink.innerHTML = '다운로드';
            		
            		fileName.style.border = fileSize.style.border = fileDate.style.border = 
            			userName.style.border = fileView.style.border = fileLink.style.border = borderStyle;
            		
            		fileName.style.verticalAlign = fileSize.style.verticalAlign = fileDate.style.verticalAlign = 
            			userName.style.verticalAlign = fileView.style.verticalAlign = fileLink.style.verticalAlign = 'middle';
            		
            		fileName.style.padding = fileSize.style.padding = fileDate.style.padding = 
            			userName.style.padding = fileView.style.padding = fileLink.style.padding = '10px';

            		tr.appendChild(fileName);
            		tr.appendChild(fileSize);
            		tr.appendChild(fileDate);
            		tr.appendChild(userName);
            		connection.fileViewer ? tr.appendChild(fileView) :'';
            		tr.appendChild(fileLink);
            		
            		table.appendChild(tr);
            		
            		connection.filesContainer.insertBefore(table, connection.filesContainer.firstElementChild);
            	}
        		...
        		var fileView = document.createElement('td');
        		...
        		fileName.innerHTML = '<strong style="color:white;">' + file.name + '</strong>';
        		fileSize.innerHTML = '<strong style="color:white;">' + formatSizeUnits(~~file.size) + '</strong>';
        		fileDate.innerHTML = '<strong style="color:white;">' + date + '</strong>';
        		userName.innerHTML = '<strong style="color:white;">' + (file.userid ? file.userid : connection.userid) + '</strong>';
        		
        		if (file.type && file.type.indexOf('image') != -1) {
    				fileView.innerHTML = '<img src="' + file.url + '" title="' + file.name + '" style="max-width: 80%;">';
    			} else {
    				fileView.innerHTML = '<iframe src="' + file.url + '" title="' + file.name + '" style="width: 80%;border: 0;height: inherit;margin-top:1em;"></iframe>';
    			}
        		
        		var link = document.createElement('a');
        		link.href = file.url;
        		link.download = file.name;
        		link.innerHTML = '<strong style="color:white;">DOWNLOAD</strong>';

        		fileLink.appendChild(link);
        		
        		tr.id = 'shared_' + file.uuid + ( file.userid ? file.userid : connection.userid );
        		...
        		// ### file sync list ###
        		if(!connection.syncFileList){
        			connection.syncFileList = {};
        		}
        		
        		connection.syncFileList[file.uuid] = {
        			share : file.userid,
        			remote : file.remoteUserId,
        			file : connection.shareFileInServer ? file : ''
        		};
            	// ######################
        		
        		// ## Notice Event
            	connection.onnoticemessage(( file.userid ? file.userid : connection.userid ), 'shareFile', file);
            ...
            // ## stop share & delete file ##
            connection.stopOrDel = function(type, userid, uuid){
            	if(type == 'shared'){
            		
            		var element = document.getElementById('shared_' + uuid + userid);
            		...
            		if(uuid){
            			...
            			var element = document.getElementById('sharing_' + uuid);
            			...
            			if(userid){
            				eleList = document.getElementsByClassName("sharing_" + userid);
            			}else {
            				eleList = document.getElementsByClassName("fileSharing");
            			}
            			
            			if(eleList){
            				var length = eleList.length;
            				for(var i=0;i<length;i++){
            					var element = eleList[0];
            					
            					// ## 파일 게시자를 제외한 사용자 fbr.chunks & fbr.users 삭제
//            					var uuid = element.querySelector('input').value;
            					
//            					if(connection.fbr.chunks[uuid][0].userid != connection.userid){
//	            					delete connection.fbr.users[userid][uuid];
//	            					delete connection.fbr.chunks[uuid];
//            					}
    ...
    (function(connection) {
            ...
            connection.socket.emit('join-room', {
                ...
                if (isRoomJoined === true) {
                    ...
                    if (!!connection.peers[connection.sessionid]) {
						...
						console.error('on socket disconnect & reconnect');
                        return;
                    }
        ...
        // ## Notice Event
        connection.onnoticemessage = function(remoteUserId, type, data){
        	console.log(remoteUserId, type);
        };
        ...
        // ################### fileSync ###################### 
        connection.syncFile = function(remoteUserid, syncFileList){
        	
        	if(!connection.syncFileList){
        		connection.syncFileList = {};
        	}
			
			if(!connection.synchronizeFils){
        		connection.synchronizeFils = {};
        	}
			
        	if(!syncFileList || Object.keys(syncFileList).length == 0 || Object.keys(syncFileList).length == Object.keys(connection.syncFileList).length) return;

        	for(var key in syncFileList){
        		
        		if(syncFileList[key].file) connection.addFile(syncFileList[key].file);
        		
        		if(connection.synchronizeFils[key] || connection.syncFileList[key] || connection.fbr.chunks[key]) continue;
				
				connection.synchronizeFils[key] = syncFileList[key];
        		
        		connection.peers[remoteUserid].channels.forEach(function(channel) {
					channel.send(JSON.stringify({ 
						message : JSON.stringify({ syncFile : key }),
						last : true,
						isobject : true,
						uuid : key,
						type : 'text'
        ...
        connection.onUserStatusChanged = function(event, dontWriteLogs) {
        	...
        	// ## stream ended ( remove element [select, video] )
        	if(event.status === 'offline'){
				connection.onstreamended({offlineId:event.userid});

				// ## Sharing Stop Evet
				if(connection.enableFileSharing && connection.stopOrDel){
					connection.stopOrDel('sharing', event.userid);
				}
        	}
        	
        	// ## Notice Event
        	connection.onnoticemessage(event.userid, 'userState', event.status);

        	if (!!connection.enableLogs && !dontWriteLogs) {
				// ## FireFox Force Reload ..... ( Peer Candidata Change Event Not Working )
        		// if(event.status === 'offline' && isFirefox) location.reload();
        ...
        connection.onPeerStateChanged = function(state) {
            if (connection.enableLogs) {
                if (state.iceConnectionState.search(/closed|failed/gi) !== -1) {
                	// @@ log
                	// console.log('Peer connection is closed between you & ', state.userid, 'state:', state.iceConnectionState);
                }
            }
        };
        ...
        connection.onSettingLocalDescription = function(event) {
            if (connection.enableLogs) {
            	...
            	// ## Notice Event
				connection.onnoticemessage(event.userid, 'userConnection', 'Set local description for remote user');
        ...
        connection.onSocketDisconnect = function(event) {
            if (connection.enableLogs) {
            	...
            	// connection.stopOrDel('sharing');
            	connection.onnetworkerror();
            	connection.onnoticemessage('your', 'networkError', 'socket.io connection is closed');
            }
        };
        ...
        connection.onSocketError = function(event) {
            if (connection.enableLogs) {
            	...
            	connection.onnetworkerror();
            	connection.onnoticemessage('your', 'networkError', 'socket.io connection is failed');
            }
        };
        
        connection.onnetworkerror = function(){}
